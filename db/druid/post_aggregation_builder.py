...
class FormulaError(Exception):
    def __init__(self, formula):
        error_msg = f'Formula cannot be parsed: {formula}'
...
class UnsupportedNodeError(Exception):
    def __init__(self, node_type):
        error_msg = f'Node type is not supported: {node_type}'
...
# NOTE: Should we maintain both types? Even though "expression" isn't
...
def test_expression_formula_valid(formula_str: str) -> bool:
    ...
    try:
        root = _parse_formula(formula_str)
        _ = _build_from_node(root)
    except (FormulaError, UnsupportedNodeError, SyntaxError):
        return False
    return True
...
def walk_post_aggregator(post_aggregator, callback):
    ...
    post aggregator.
    '''
    # NOTE: Pydruid stores the post_aggregator dict as an instance on
...
def add_suffix_to_expression(expression, suffix):
    ...
    for match in FIELD_MATCH_PATTERN.finditer(expression):
        ...
        pieces.append(f'{match.group()}{suffix}')
        prev = end
