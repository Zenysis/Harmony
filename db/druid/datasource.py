...
class SiteDruidDatasource(DruidDatasource):
    ...
    # NOTE: Keep the date portion regex in sync with the date format
    DATASOURCE_PATTERN = re.compile('((?:[a-z0-9]+_?)+)_([0-9]{8})')

    # TODO: Change this to '%Y%m%d.%H%M%S' to prevent collisions when
    # multiple datasources are built on the same day.
    DATE_FORMAT = '%Y%m%d'

    __slots__ = ('_date', '_site_name')
    ...
    def __init__(self, site_name, datasource_date=None):
        ...
        datasource_name = f'{self._site_name}_{date_str}'
    ...
    # NOTE: these three (`_as_tuple`, `__hash__` and `__eq__` are necessary
    # so this becomes a valid argument for a function that is being `@lru_cache` decorated
    def _as_tuple(self):
    ...
    def __eq__(self, other):
        if isinstance(other, SiteDruidDatasource):
    ...
    def _validate_datasource_name(cls, datasource_name):
        ...
        if not match or len(match.groups()) != 2:
            error_str = f'Pattern mismatch: {cls.DATASOURCE_PATTERN.pattern}'
        else:
