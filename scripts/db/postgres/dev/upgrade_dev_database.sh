...
DB_EXISTS_TEST=$(psql -h ${POSTGRES_HOST:-localhost} -U ${POSTGRES_USER:-postgres} ${POSTGRES_DB:-postgres} -tAc "SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'")

if [[ "${DB_EXISTS_TEST}" == '' ]] ; then
  echo "Creating database ${DB_NAME}"
  psql -h ${POSTGRES_HOST:-localhost} -U ${POSTGRES_USER:-postgres} ${POSTGRES_DB:-postgres} -tAc "CREATE DATABASE \"${DB_NAME}\""
fi
...
# version.
# NOTE: Filtering out some common log messages so that databases that
# don't need upgrading will not produce any logs.
# NOTE: Using the simplified base app since we don't need to import all
# the extra crap that is needed to actually start the full flask server. We only
# need the core.
export DATABASE_URL="postgresql://${POSTGRES_USER:-postgres}:${POSTGRES_PASSWORD}@${POSTGRES_HOST:-localhost}/${DB_NAME}"
