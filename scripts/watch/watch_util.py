...
def script_already_running(script_name: str) -> bool:
    ...
    pid_str = check_output('pgrep -af "%s"' % script_name, shell=True, text=True)
...
# TODO(stephen): You've created yet another bespoke general process running
...
def exec_ignore_exception(func, exception_type, count=0, max_catch=5):
    ...
    and quit.
    NOTE(stephen): There may be a way to do this with signal handlers.
    '''
    try:
        func()
