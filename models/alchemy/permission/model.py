...
class ResourceTypeEnum(Enum):
    ...
    # A resource type that represents a role on the site
    ROLE = 7
...
class Permission(Base):
    ...
    # TODO - Make this a mixin to keep the model class 'pure'
    def build_permission(self, resource):
        # NOTE: Lazily import ItemNeed since this class can be
...
class Role(Base):
    ...
    def invalidate_involved_users_permission_caches(self):
        """
        When a role is changed there's a need to update the permission caches.
        For role those can be users directly attached to this role or through a
        security group.
        """
        # TODO: this is not very efficient and should actually just make
        # and SQL query that retrives all the users in one go instead of serializing
        # so many things around. But now need the fix to be quick.
        users = {}
        ...
        for user in users.values():
            after_user_role_change.send(user, role=self)
