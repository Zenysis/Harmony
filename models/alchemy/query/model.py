...
class Field(Base, UTCTimestampMixin):
    ...
    copied_from_field_id = sa.Column(
        sa.String(),
        sa.ForeignKey(
            'field.id', ondelete='CASCADE', name='copied_from_field', onupdate='CASCADE'
        ),
    )
...
class Dimension(Base, UTCTimestampMixin):
...
class FieldDimensionMapping(Base, UTCTimestampMixin):
    ...
    field_id = sa.Column(
        sa.String(),
        sa.ForeignKey(
            'field.id', ondelete='CASCADE', name='valid_field', onupdate='CASCADE'
        ),
        nullable=False,
    )
    dimension_id = sa.Column(
...
class PipelineDatasource(Base, UTCTimestampMixin):
...
class FieldPipelineDatasourceMapping(Base, UTCTimestampMixin):
...
class Category(Base, UTCTimestampMixin):
...
class FieldCategoryMapping(Base, UTCTimestampMixin):
...
class DimensionCategory(Base, UTCTimestampMixin):
...
class DimensionCategoryMapping(Base, UTCTimestampMixin):
...
class DimensionValue(Base):
    ...
    id = sa.Column(sa.Integer(), primary_key=True)
    datasources = sa.Column(ARRAY(sa.Integer), nullable=False)
    dimension = sa.Column(sa.String(), nullable=False)
    filter = sa.Column(MutableDict.as_mutable(JSONB()), nullable=False)
    name = sa.Column(sa.String(), nullable=False)
    description = sa.Column(sa.String(), nullable=False, default='')
    subtitle = sa.Column(sa.String(), nullable=False, default='')

    __table_args__ = (
        sa.UniqueConstraint('dimension', 'name', 'subtitle'),
        sa.Index('datasources_idx', datasources, postgresql_using='gin'),
    )


class DruidDatasource(Base, UTCTimestampMixin):
    __tablename__ = 'druid_datasource'

    id = sa.Column(sa.Integer(), primary_key=True)
    datasource = sa.Column(sa.String(), nullable=False, unique=True)
    min_date = sa.Column(sa.Date(), nullable=False)
    max_date = sa.Column(sa.Date(), nullable=False)
    meta_data = sa.Column(MutableDict.as_mutable(JSONB()), nullable=False)
