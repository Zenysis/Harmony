...
# NOTE: None of these defaults match our *scss* defaults and variables.
# TODO: Maybe this should be standardized somewhere
# else in the schema.
...
class Coordinates(list):
    def __init__(self, values):
        if len(values) != 2:
            raise ValueError(f'Coordinates must be length 2: {values}')

        super().__init__(self)

        # TODO: Validate that lat/lon are real geo coordinates in the
        # correct range.
...
class BarGraphSettings:
    ...
    un_pivot = related.BooleanField(False, key='unPivot')
...
class BumpChartSettings:
    ...
    # NOTE: This is a mapping from field ID to True. That is difficult
...
class EpiCurveSettings:
    ...
    # NOTE: Defaulting to the Tableau 10 color scheme.
    ...
    selected_field = related.StringField('', key='selectedField')
    # TODO: Remove this property once GroupBySettings are exposed to
...
class MapSettings:
    ...
    auto_fit_to_visible_data = related.BooleanField(False, key='autoFitToVisibleData')
    ...
    # NOTE: This setting is really poorly designed. It encodes
...
class TableSettings:
    ...
    enable_auto_expand = related.BooleanField(False, key='enableAutoExpand')
    ...
    pivoted_dimensions = related.SequenceField(str, ['field'], key='pivotedDimensions')
...
# TODO: Remove show_data_labels, it has been moved to series settings.
@related.mutable(strict=True)
class TimeSeriesSettings:
    # TODO: Define the real structure of the BandSettings as a full model.
