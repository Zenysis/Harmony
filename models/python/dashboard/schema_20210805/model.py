...
from .visualization_settings.visualization_specific_model import MapSettings
...
class GISItemDefinition(DashboardItem):
    # NOTE: We don't want to persist the data of an entity layer
    # (including its list of features) because it is costly.
    @related.mutable(strict=True)
    ...
    class IndicatorLayerDefinition:
        id = related.StringField(key='id')
        controls = related.ChildField(MapSettings, key='controls')
        visualization_settings = related.ChildField(
            ViewTypeSettings, key='visualizationSettings'
        )

        # Ambiguously defined because frontend model changes frequently
    ...
    # TODO: $GISDashboard - This is an ambiguous type that allows us
    # to pass in new properties without forcing a dashboard upgrade every time.
    # When we decide on a more stable set of settings, we can replace this with
    # a real shape
    general_settings = related.ChildField(dict, {}, key='generalSettings')
...
class PlaceholderItemDefinition(DashboardItem):
    ...
    can be added to a dashboard. When a query or iFrame or text item is added
    to them, this model is removed and replaced with the corresponding item.
    '''

    icon = related.StringField(key='icon')
...
class DashboardSpecification:
    ...
    legacy = related.BooleanField(True)
