...
class DateRangeType(Enum):
    ...
    '''
    Indicates that the date range represents two absolute points in time.
    '''
    ABSOLUTE = 0
...
class LayoutItem(PythonModel):
    ...
    @LayoutItemProperty.getter(
        ...
        default_value=lambda: {},
        value_validate_function=lambda value: assert_mapping(
            value, 'front_end_selections'
        ),
    )
    def front_end_selections(self):
        return self.get('frontEndSelections')

    @front_end_selections.setter
    @LayoutItemProperty.setter()
    def front_end_selections(self, value):
        self['frontEndSelections'] = value
