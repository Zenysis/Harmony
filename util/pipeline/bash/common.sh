...
FetchFiles () {
  ...
  # NOTE: By using an array to store the input files, we can avoid
...
UploadDirectory () {
  ...
  # NOTE: Different versions of mc handle copying data recursively
  # into a target destination differently. Older versions will copy the whole
  # folder into the destination. Newer versions copy the *contents* of the folder
  # into the destination. By entering the input directory, we can standardize the
  # handling. Even newer versions will copy the contents only when the source ends
  # with "/" and will otherwise copy the folder itself (even after entering the
  # input directory). By specifying the source as "./", we can standardize the
  # behavior across all versions.
...
# TODO: When minio-client supports globs in the copy command, remove
# this workaround.
_GetMatchingFiles () {
  local input="$1"
  local path
...
# TODO: Search "locations_header" for code that needs to be replaced.
