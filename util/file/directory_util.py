...
def _validate_dir_path(path):
    ...
    '''
    assert (
        os.path.isdir(path) and os.path.realpath(path) != '/'
    ), f'Invalid directory provided: {path}'
...
def compute_file_hash(filename):
    ...
    '''
    assert os.path.isfile(filename), f'Invalid file: {filename}'
...
def compute_dir_hash(selected_dir):
    ...
    for dir_file in FileUtils.GetFilesInDir(selected_dir):
        ...
        res.append(f'{rel_path}\t{compute_file_hash(dir_file)}')
...
def equal_dir_content(dir_a, dir_b):
    ...
    are equivalent.
    NOTE: The name equal_dir_content could be interpreted as 'exact'
