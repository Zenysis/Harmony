...
import DimensionsDigestData from 'models/DataDigestApp/DimensionsDigestData';
...
/**
 * DataDigestService is used to fetch and write files needed for the data
 * digest product.
 */
class DataDigestService {
  ...
  /**
   * Returns either the unmatched non hierarchical dimension data or the mapped
   * non hierarchical dimension data for a given DatasourceDigest.
   * Returns undefined if no dimension data could be found.
   *
   * @param {DatasourceDigest} digest The DatasourceDigest object we want to
   * pull dimension data from.
   * @param {'mapped' | 'unmatched'} matchState Whether or not to pull mapped
   * or unmatched dimension data.
   * @returns {DimensionsDigestData | void}
   */
  _getDimensionsData(
    digest: DatasourceDigest,
    matchState: 'mapped' | 'unmatched',
  ): Promise<DimensionsDigestData | void> {
    const filepath =
      matchState === 'mapped'
        ? digest.getMappedDimensionsFilepath()
        : digest.getUnmatchedDimensionsFilepath();
    if (filepath) {
      return this.getDataDigestFile(filepath).then(rows =>
        DimensionsDigestData.deserialize(rows, {
          filepath,
          datasourceName: digest.datasourceName(),
          treatAllDimensionsAsMapped: matchState === 'mapped',
        }),
      );
    }
    return Promise.resolve(undefined);
  ...
  /**
   * Returns the unmatched non hierarchical dimension data for a given
   * DatasourceDigest.
   * Returns undefined if no unmatched non hierarchical dimension data
   * could be found.
   *
   * @param {DatasourceDigest} digest The DatasourceDigest object we want to
   * pull dimension data from.
   * @returns {DimensionsDigestData | void}
   */
  getUnmatchedDimensionsData(
    digest: DatasourceDigest,
  ): Promise<DimensionsDigestData | void> {
    return this._getDimensionsData(digest, 'unmatched');
  }

  /**
   * Returns the mapped non hierarchical dimension data (i.e. all dimensions
   * with a correct match) for a given DatasourceDigest.
   * Returns undefined if no mapped non hierarchical dimension data could be
   * found.
   *
   * @param {DatasourceDigest} digest The DatasourceDigest object we want to
   * pull dimension data from.
   * @returns {DimensionsDigestData | void}
   */
  getMappedDimensionsData(
    digest: DatasourceDigest,
  ): Promise<DimensionsDigestData | void> {
    return this._getDimensionsData(digest, 'mapped');
  }
