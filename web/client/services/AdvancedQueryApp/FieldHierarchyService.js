...
class FieldHierarchyService {
  ...
  > = HierarchyItem.create({
    ...
    metadata: LinkedCategory.create({
      id: MRU_NODE_ID,
      name: I18N.text('Most Recently Used Indicators'),
    }),
  });
   ...
   *
   * NOTE: This call is memoized so that we only do this work once.
  ...
  syncPersistentMRUCache(allowSyncingEmptyList: boolean = false): void {
    ...
    // NOTE: There is an implicit dependency that the HierarchyItem's
