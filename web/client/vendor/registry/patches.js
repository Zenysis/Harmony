...
export function patchPlotlyHoverEvent() {
  ...
  function patchEvent(event: mixed): mixed {
    ...
    // NOTE: Take advantage of how VX has already solved this issue and
    // we can just use their library.
    const eventLocalCoordinates = localPoint(event);
    const targetLocalCoordinates = target.getCTM();

    // NOTE: Probably can be an invariant, but since we're patching a
  ...
  function maybeScaleTooltipBackground(event: mixed) {
    ...
    // NOTE: Plotly stores the hover layer as a completely separate
    ...
    // inversion applied.
    // NOTE: This is the most reliable way to reposition the text
    ...
    textPaths.forEach(text => {
      ...
      // NOTE: The font-size used here is 13px since this is a Plotly
...
// NOTE: This overrides D3's format function in align with the one in
