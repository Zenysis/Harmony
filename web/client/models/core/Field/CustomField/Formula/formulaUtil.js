 ...
 *
 * NOTE: This function has SIDE EFFECTS and modifies a given data frame
 ...
 *
 * NOTE: if the rows include timestamps, we do not do any special
...
export function buildDataFrameFromTimeSeriesRows(
  ...
  // using a lot of mutability to keep things efficient
  rows.forEach(dataObj => {
    const { dimensions, key, timestamp, ...fieldValues } = dataObj;
    const row: { [id: string]: string | number | void, ... } = {};
