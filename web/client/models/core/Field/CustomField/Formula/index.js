...
function _buildAST(formulaText: string): AST {
  try {
    ...
    // NOTE: The statement does not need to be a single line or a
    ...
    // function.
    // NOTE: The only parse error raised by acorn that requires a
...
class Formula extends Zen.BaseModel<Formula, Values, {}, DerivedValues> {
      ...
      formula => {
        ...
        return metadata.sortedFields().reduce((finalText, field) => {
          ...
          if (fieldConfig && fieldConfig.treatNoDataAsZero) {
            // NOTE: It is possible for field IDs to be substrings of
            ...
            // NOTE: With advanced custom calcs, fields may be referenced
  ...
  _prepareInterpreterForEval(environment: Environment): void {
    ...
    // TODO: pull all of these from a registry of helper functions.
  ...
  /**
   * NOTE: preparing the dataframe to a pseudo-object that can be used
   ...
   * NOTE: we intentionally return `null` here instead of `void`
  ...
  evaluateFormula(dataFrame: DataFrame): Array<number | null> {
    ...
    if (this.referencesDataFrame()) {
      const sortedDataFrameInfo = sortDataFrame(
        ...
        // TODO: eventually if advanced custom calcs gets generalized
