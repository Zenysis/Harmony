...
export default class QueryResultState<
  ...
  // NOTE: this type is not filled in by the user, it is computed by
...
> {
  static createInitialState(
    ...
    // NOTE: Need to explicitly specify the type here since Flow is
  ...
  /**
   * Build and run a new query from the provided Selections and QueryResultSpec,
   * BUT the queryEngine.runRawQuery adds all of the location hierarcy as dimensions
   * to group by and if no granularity dimension is added, it adds the day granularity.
   * Returns a Promise that resolves a new QueryResultState instance with the
   * new queryResult, this is un-aggregated data.
   */
  // NOTE: This is not how we should separate a 'raw query' from others
  // Fix this asap.
  runRawQuery(
    querySelections: QuerySelections,
    queryResultSpec: QueryResultSpec,
    ...
    return (
      this._values.queryEngine
        // $FlowFixMe[prop-missing]
        .runRawQuery(querySelections)
        .then((rawQueryResult: Zen.Serialized<QueryResultDataModel>) => {
          // If the query failed to return results, clear the current state.
          if (rawQueryResult === undefined) {
            return this.clear();
