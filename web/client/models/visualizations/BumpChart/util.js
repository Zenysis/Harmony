 ...
 * If the first date doesn't match, move more recent timstamps down.
 */
export function sortLines(lines: Array<LineData>): Array<LineData> {
  const compareStringDates = (
    timeStampA: string,
    timeStampB: string,
  ): number => {
  ...
  return lines.sort((a, b) =>
    a[0].timestamp !== b[0].timestamp
      ? compareStringDates(a[0].timestamp, b[0].timestamp)
      : a[0].rank - b[0].rank,
  );
}
...
function _sortValues(
  ...
  if (diff !== 0) {
    // NOTE: Sort descending only applies when we don't have a tie.
...
export function parseQueryResult(
  ...
  // Extract a list containing a unique list of keys. Sory by field id. Find the
  // top X keys in the first bucket since that is the start of the graph.
  // We disregard date entirely, since we may not have a record for the start date.
  ...
  const firstBucketData = data
    ...
    .filter(value => dates.includes(value.timestamp))
  ...
  data.forEach(d => {
    ...
    // NOTE: This is a short label used by the y-axis. Figure out
