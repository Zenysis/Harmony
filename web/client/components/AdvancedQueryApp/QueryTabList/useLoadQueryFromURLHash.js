...
function generateNewTabName(
  ...
  let tabName = I18N.text('Copy from %(username)s', {
    username: querySession.username(),
  });
  let copyCount = 1;
  while (allTabNames.has(tabName)) {
    copyCount += 1;
    tabName = I18N.text('Copy %(num)s from %(username)s', {
      num: copyCount,
      username: querySession.username(),
    });
  }
...
export default function useLoadQueryFromURLHash(): (
  ...
  // hash
  return React.useCallback((currentTabList: Zen.Array<QueryTabItem>) => {
    ...
    return QuerySessionService.getQuerySession(serverSessionId)
      ...
      .catch(e => {
        // TODO: Consider removing this entry from DB
        notifyError(
          I18N.text(
            'Link was not valid. Please ensure it was correct',
            'linkNotValid',
          ),
          e,
        );
