...
// TODO: Can this be configured globally?
...
> extends React.PureComponent<Props<ViewType>, State> {
  ...
  titleResizeRegistration: ResizeRegistration<HTMLDivElement> = ElementResizeService.register(
    this.onChangeTitleHeight,
  );
  ...
  componentDidUpdate(prevProps: Props<ViewType>) {
    ...
    // NOTE: Quick check to avoid updating when only the loading state
  ...
  // queryResultSpec.
  // NOTE: This would be a good place to use the memoizeOne decorator.
  processChanges(
  ...
  @autobind
  onChangeTitleHeight({ contentRect }: ResizeObserverEntry) {
    this.setState({ titleHeight: contentRect.height });
  }
  ...
  render(): React.Node {
    ...
    return (
      ...
      >
        <div ref={this.titleResizeRegistration.setRef}>
          {this.renderTitle()}
        </div>
        {vizContent}
      </div>
