...
// NOTE: This is all just a mess. Creating a new line type type
...
// NOTE: To the person that refactors the LineGraph to stop using
...
class LineGraph extends React.PureComponent<Props, State> {
  ...
  // NOTE: Just going full-on hack mode at this point with Plotly
  ...
  buildMetricOrder(
    ...
    seriesSettings.visibleSeriesOrder().forEach(fieldId => {
      ...
      if (seriesObject !== undefined) {
        ...
        // NOTE: If there is only a single row in the query result, we
        ...
        // TODO: Improve Flow's refinement of this.
        // $FlowExpectedError[incompatible-type]
        const shape: $PropertyType<Metric, 'shape'> = `line-${plotlyDashStyle}`;

        // NOTE: BR COVID has very specific requirements around the
        ...
        if (hasSingleQueryResultRow && HACK_USE_SERIES_SETTING_COLOR) {
          // NOTE: The series object color should never be missing, but
   ...
   * TODO: Remove this method when the timestamp grouping is
  ...
  buildDefaultDateQueryResultGrouping(bucketType: string): QueryResultGrouping {
    return QueryResultGrouping.create({
      // NOTE: Under rare circumstances, this path can get hit in AQT.
  ...
  buildDateGrouping(
    ...
    // NOTE: This parameter only matters for SQT. Remove it when SQT
   ...
   */
  // NOTE: Memoizing purely based on the props since this code is a
  ...
  buildPlotlyData(
    ...
    // TODO: Something about coloring lines differently if no grouping
    ...
    // NOTE: Draw the lines *last* so that they appear on top of
    ...
    data.forEach(dataPoint => {
      ...
      for (let i = 0; i < dataPoint.y.length; i++) {
        ...
        const t = {
          ...
          // NOTE: We have to adjust log scale the y-value ourselves
          // to work around a bug in the Plotly library.
          // NOTE: The text padding adjustment doesn't do very much
  ...
  getDataPointAdjustments(
    ...
    y2: number | void,
    y3: number | void,
  ): { adjustYDirection: 'top' | 'bottom' | '', xAnchor: string } {
    const top = { adjustYDirection: 'top', xAnchor: 'center' };
    const bottom = { adjustYDirection: 'bottom', xAnchor: 'center' };
    const rightSide = { adjustYDirection: '', xAnchor: 'left' };
    const leftSide = { adjustYDirection: '', xAnchor: 'right' };
  ...
  // NOTE: Manually compute the tick labels to display instead of
  ...
  buildTickValues(
    ...
    // NOTE: This spacing calculation is copied from Plotly.
    ...
    // NOTE: Try to space the date values cleanly when bucketing by
    ...
    return dateLabels.filter((label, idx) => {
      // NOTE: Parse the simple date string and determine if the year
  ...
  getGraphLayout(height: number, width: number): $FlowTODO {
    ...
    const layout = {
      ...
      hovermode: 'closest',
      // NOTE: Disabling plotly's legend since we render our own
      // version.
      showlegend: false,
    ...
    if (y1AxisSeries.length > 0) {
      ...
      // NOTE: Plotly can't have yaxis be undefined
      delete layout.yaxis;
    }
    ...
    if (y2AxisSeries.length > 0) {
      ...
      // NOTE: Plotly can't have yaxis2 be undefined
  ...
  renderPlot(height: number, width: number) {
    // NOTE: This is so dirty. But I've given up on making the Plotly
    // line graph nice.
    ...
    // NOTE: Since the Plotly plot requires height/width values to be
