...
export default class QueryInterface<Request, Response> {
  ...
  run(): Promise<Response> {
    ...
    const requestPromiseThunk = (): Promise<Response> =>
      APIService.post(this.apiVersion, this.endpoint, this._request).catch(
        e => {
          // eslint-disable-next-line no-console
    ...
    const promise = Promise.using(queuedPromise, response => {
      ...
      // TODO: Update visualizations to show useful error messages
      // when this happens.
  ...
  // TODO: JSON.stringify is not stable, so we could generate
