...
function getLabelValues(
  ...
  Object.keys(labelProperties).forEach(labelId => {
    if (!disabledLabels.has(labelProperties[labelId].label)) {
      ...
      // NOTE: We can only append strings, so we need to have a string
...
function DataPointLabelLayer({
  ...
  // NOTE: I originally wanted to just reference an existing source
  ...
  // TODO: Figure out how to customize the fonts for mapboxgl. They
  ...
  if (centerAlign) {
    layoutProps['text-justify'] = 'center';
    layoutProps['text-radial-offset'] = 0;
    layoutProps['text-variable-anchor'].unshift('center');
  }
  const layerStyle = {
    id,
    layout: {
