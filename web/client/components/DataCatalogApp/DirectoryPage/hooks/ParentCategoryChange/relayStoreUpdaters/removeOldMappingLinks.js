...
// NOTE: Right now, this method requires tha the original parent item
// have a single mapping field inside it that leads to the child field.
// Example: parent { parent_to_child_mapping { child { id } } }
// NOTE: You should not call this method if your mutation has remapped
...
export default function removeOldMappingLinks(
  ...
  // NOTE: This should not normally happen, but we need to be really
  ...
  const updatedMappingRecords = originalMappingRecords.filter(record => {
    // NOTE: Allowing null/undefined records to be preserved since this
    ...
    // NOTE: Same comment as preserving null/undefined records as
    // above.
