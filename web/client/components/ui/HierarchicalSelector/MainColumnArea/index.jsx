...
const ANIMATION_CONFIG = { ...config.default, clamp: true, precision: 0.05 };
...
> {
  ...
  setNewScrollPosition(prevProps: Props<T>) {
    ...
    this.setState((state, props) => {
      const { hierarchyItems, searchResults } = props;
      const addedColumn =
  ...
  switchFadedItemColumns() {
    this.setState(state => {
      const { columnToFadeIn, columnToFadeOut, columnsToAnimate } = state;
  ...
  getSearchColumnWidth(): number {
    const { columnWidth, maxWidth } = this.props;
    return AnimatableColumns.getSearchColumnWidth(columnWidth, maxWidth);
  }

  getExpectedScrollPosition(): number {
    const { columnWidth, hierarchyItems, maxWidth, searchResults } = this.props;
  ...
  onSearchColumnCategoryClicked(categoryPath: Zen.Array<HierarchyItem<T>>) {
    ...
    this.setState(prevState => {
      const { columnsToAnimate, searchColumn } = prevState;
  ...
  maybeRenderColumnTitlesRow(): React.Node {
    const { columnTitleGenerator, columnWidth } = this.props;
  ...
  renderItemColumn(hierarchyItem: HierarchyItem<T>): React.Node {
    ...
    const { hierarchyItems, maxHeight, testItemSelectable } = this.props;
    const { columnHeight } = this.state;
