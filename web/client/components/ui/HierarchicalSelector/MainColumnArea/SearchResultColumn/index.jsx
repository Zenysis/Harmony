...
> extends React.PureComponent<Props<T>, State> {
  ...
  getTotalSearchResults(
    ...
    return searchResults.reduce(
      // NOTE: we add `+ 1` because the result group itself counts
  ...
  renderSearchResults(): React.Node {
    const { matcher, onCategoryClick, onItemClick, searchResults } = this.props;
    ...
    for (let i = 0; i < searchResults.length; i++) {
      const { items, key, path } = searchResults[i];
  ...
  renderNoSearchResults(): React.Node {
    return (
      <div className="hierarchical-search-no-results">
        {I18N.text('No Search Results')}
      </div>
    );
  }
