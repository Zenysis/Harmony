...
type DefaultProps = {
  ...
  // TODO: convert this to a render prop
  ...
  /** Class name for styling the React Overlay component */
...
export default class BaseModal extends React.PureComponent<Props, State> {
  static defaultProps: DefaultProps = {
    ...
    secondaryButtonText: I18N.textById('No'),
  ...
  componentDidMount() {
    // NOTE: mobile browsers (e.g. Safari and Chrome on iOS) have a very
  ...
  maybeRenderPrimaryButton(): React.Node {
    ...
    if (primaryButtonTooltip) {
      return (
        <Tooltip content={primaryButtonTooltip} tooltipPlacement="top">
          {button}
        </Tooltip>
      );
    }
  ...
  maybeRenderSecondaryButton(): React.Node {
    ...
    if (secondaryButtonTooltip) {
      return (
        <Tooltip content={secondaryButtonTooltip} tooltipPlacement="top">
          {button}
        </Tooltip>
      );
    }
...
if (document.getElementById('main')) {
  // if the modal is loaded:
  Modal.setAppElement('#main');
} else if (document.getElementById('rsg-root')) {
  // if we're rendering in styleguide:
