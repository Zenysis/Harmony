 ...
 * NOTE: Half has to be calculated differently than quarter/year since
 ...
 */
// TODO: this should no longer be exported once the old date picker is
// killed
...
function _getRangeForLastDateUnit(
  ...
  switch (dateConfig.dateUnit) {
    ...
    case 'FISCAL_YEAR': {
      ...
      return {
        from,
        // NOTE: the end date returned by the buildFiscalDates function
...
export default function computeDateRange(
  ...
  const { fiscalStartMonth, maxAllTimeDate, minAllTimeDate } =
    dateConstraints || {};
  ...
  switch (dateConfiguration.modifier) {
    ...
    case 'YEAR_TO_DATE': {
      const { numYearsLookback, usePreviousYear } = dateConfiguration;
      const lookbackYears =
