...
export default class Pie extends React.PureComponent<Props> {
  ...
  // in the pie.
  // NOTE: Calculating every position instead of only the highlighted
  ...
  // container.
  // NOTE: Using a scale transform vs recomputing the pie dimensions
  ...
  buildScale(
    ...
    labelData.forEach(({ position }) => {
      // NOTE: Since the scaling will be applied equally to all sides
    ...
    // NOTE: Choosing to consider the case when *all* labels are shown
  ...
  maybeRenderLabels():
    ...
    return highlightedLabelData.map(({ displayValue, key, position }) => (
      <text
        key={key}
  ...
  renderPieSegments(): React.Node {
    ...
    const arcPaths = arcs.map(arc => {
      ...
      // TODO: On segment hover, maybe show value, percent,
      return (
        <g key={arc.data.key}>
          <path
            d={path(arc)}
