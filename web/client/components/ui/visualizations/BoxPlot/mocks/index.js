...
// NOTE: Track the box plots generated so that when the user changes
...
export function generateBoxPlotData(
  ...
  for (let group = start; group < numberOfGroups; group += 1) {
    ...
    const outliers = points
      ...
      .map(value => ({
        value,
        dimensions: { Facility: 'Some Facility', Region: 'Some Region' },
      }));
    ...
    GENERATED_BOXES.push({
      data: { binData, boxPlotSummary, outliers },
      key: `Group ${group}`,
    });
  }
