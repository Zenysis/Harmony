...
type State = {
  ...
  // NOTE: Would be nice to have a Zen.Set here.
...
// NOTE: Need to use `display: block` on the focus window SVG to ensure
...
// NOTE: Need to apply `display: block` to the root SVG when in
...
export default class BarGraph extends React.PureComponent<Props, State> {
  ...
  // NOTE: The clip path ID *must be unique*. If the BarGraph component
  ...
  buildVisibleDataPoints(
    ...
    return sortedDataPoints.filter(dataPoint => {
      // NOTE: Slightly unsafe access of `this` property inside
  ...
  buildAxisTopHeight(
    ...
    // NOTE: Implement a simple heuristic to try and minimize the
    ...
    const maxValueLength = metricOrder.reduce((curMax, metric) => {
      ...
      // NOTE: Since we don't know which metric has the largest value,
      ...
      // NOTE: We could directly calculate it by finding the max value
      ...
      // NOTE: The only reason we would store the value length if the
    ...
    // TODO: When value font size is implemented, we will need a better
  ...
  usingFocusWindow(): boolean {
    const { defaultVisibleBarGroups, enableFocusWindow } = this.props;

    // NOTE: Right now, the horizontal bar chart cannot use the focus
    // window.
  ...
  buildScaleMap(
    ...
    return { barGroupScale, barScale, y1Scale, y2Scale };
  }
  ...
  getAxisLayers(): $ReadOnlyArray<LayerData> {
    ...
    // NOTE: Passing in `selectedDimensionValues` even though it is
  ...
  ): {| cursor?: string, fontWeight?: 'bold' | number |} {
    const output = {
  ...
  onYAxisHoverMove(axis: YAxisID, value: number) {
    const hoverLine = createGoalLineObject({ axis, value, id: 'hover' });
  ...
  /**
   * NOTE: We want to round values so that they display nicely on
  ...
  onFocusAreaChange(start: number, end: number) {
    this.setState({ focusPosition: { end, start } });
  }
  ...
  onInnerChartResize(height: number, width: number) {
    this.setState(prevState => {
      ...
      // NOTE: COME BACK TO THIS HACK HACK HACK
      ...
      // NOTE: The chart has a nonzero initial size. paramaterize this.
  ...
  maybeRenderFocus(): React.Node {
    ...
    const { end, start } = this.state.focusPosition;
    ...
    // NOTE: The `bandwidth` of the bar scale is the width that each
    ...
    // NOTE: When we hide the bars, we will draw a gray box along the
    ...
    // NOTE: Since we are rendering the FocusWindow outside of the
  ...
  // NOTE: It is very difficult to ensure that bar values can be
  ...
  renderInnerChart(): React.MixedElement {
    ...
    // NOTE: Passing `selectedDimensionValues` into the `BarSeries`
    ...
    // NOTE: Increase the height of the clip-path so that when bar
  ...
  renderGraph(): React.Node {
    ...
    // NOTE: We unfortunately cannot apply the `transform` directly to
      ...
      >
        ...
        >
          <ResponsiveContainer
            ariaName={fullAriaName}
            axisBottom={this.maybeRenderXAxis()}
            // NOTE: Flip the y-axes when the chart is in horizontal
