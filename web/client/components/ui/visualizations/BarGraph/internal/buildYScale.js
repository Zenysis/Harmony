...
export default function buildYScale(
  ...
  dataPoints.forEach(({ metrics }: DataPoint) => {
    metricOrder.forEach(({ id, visualDisplayShape }) => {
      // NOTE: Even though Number.isFinite properly refines the
  ...
  // NOTE: I believe the only way this is possible is if the min and
  // max are both 0.
