...
function UnpublishedFieldRow({
  ...
  const onPublishClick = React.useCallback(() => {
    ...
    if (requiredfieldsCompleted) {
      commit({
        onCompleted: () => {
          Toaster.success(
            I18N.text('Successfully published indicator %(name)s', { name }),
          );
        },
  ...
  const renderDatasource = () => {
    if (unpublishedFieldPipelineDatasourceMappings.length === 0) {
      return (
        <span className="fs-field-row__datasource--error">
          {I18N.text('Missing data source')}
        </span>
      );
    }
    // Fields only map to 1 datasource currently so we are using the first mapping.
    const datasource = unpublishedFieldPipelineDatasourceMappings[0];
    if (datasource.datasource.name) {
      return datasource.datasource.name;
    }
    // Use the id if the datasource doesn't have a name.
    return datasource.pipelineDatasourceId;
  };
