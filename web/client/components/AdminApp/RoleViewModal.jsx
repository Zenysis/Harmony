...
const TOOL_NAMES_TEXT_MAPPING = {
  alertsApp: I18N.text('Alerts'),
  analyzeTool: I18N.textById('Analyze'),
  dashboardCreator: I18N.textById('Dashboards'),
  dataQualityLab: I18N.text('Data quality'),
};
...
const policyDimensions = window.__JSON_FROM_BACKEND.queryPolicyDimensions;
...
export default function RoleViewModal({
  ...
  const [
    ...
    role !== undefined
      ...
      : I18N.textById('dashboardRequireInvite'),
  );
  const [
    ...
    role !== undefined
      ...
      : I18N.textById('alertRequireInvite'),
  );
  ...
  // NOTE: The loadedData is used to check when the state has been
  ...
  const onPrimaryAction = () => {
    // TODO: when ZenMaps allow non-string keys this cast won't be necessary
    ...
    policyDimensions.forEach(policyDimension => {
      ...
      if (policyRadioMap.get(policyDimension)) {
        // NOTE: Since the `all` property is initialized to `undefined`
        ...
        dimensionPolicies.policies.forEach(policy => {
          if (selectedPolicies.includes(policy.uri())) {
  ...
  const renderToolsCards = () => {
    const cards = Object.keys(DEFAULT_TOOLS_SELECTED).map(toolName => {
      ...
      return (
        ...
        >
          {checkmark}
          <div className="create-role-modal__card-header">
            {TOOL_NAMES_TEXT_MAPPING[toolName]}
          </div>
          <Icon
