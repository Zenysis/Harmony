...
export default function useAugmentedDashboard(
  ...
  // TODO: Currently we only fetch the filter and grouping items. We
  ...
  const loadingStatus =
    modifiersLoaded && localStorageDashboardSpecLoaded && currentDashboard
      ? 'success'
      : 'loading';

  // NOTE: We only want this to run *once* per page load. Augmented
  ...
  // NOTE: Intentionally excluding the session `filters` and
  ...
  return [currentDashboard, setCurrentDashboard, loadingStatus];
}
