...
export default function useSavedDashboard(
  ...
  React.useEffect(() => {
    ...
    // NOTE: If the user is in the legacy experience, we must change
    ...
    const promise = DashboardService.getDashboardByUri(fullURI)
      ...
      .catch(error => {
        // NOTE: This does not implement the "fallback dashboard"
  ...
  // NOTE: Certain CSS styles should only be applied in legacy mode
