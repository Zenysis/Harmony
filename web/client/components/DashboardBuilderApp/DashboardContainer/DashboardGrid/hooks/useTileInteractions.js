...
function buildLayoutAfterDrag(
  ...
  // caused by dragging.
  // NOTE: This is an optimization. Technically, we could just perform
...
export default function useTileInteractions(): [
  ...
  const onDrag = React.useCallback(
    (
      ...
      // NOTE: This isn't actually possible, but the RGL types are
      ...
      // NOTE: To avoid performing potentially costly compaction every
  ...
  React.useEffect(() => {
    ...
    // NOTE: We are using `setTimeout` here so that we can attach the
  ...
  // NOTE: Add a body-level class when the user is editing so that we
