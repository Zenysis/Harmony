...
from config.general import SUPPORT_EMAIL
...
SUPPORT_CONTACT = f'We\'re ready to assist, email us at {SUPPORT_EMAIL}'
...
class EmailRenderer:
    def __init__(
        ...
        self.deployment_short_name = deployment_short_name
    ...
    def bcc_project_managers(self, email_msg):
        for user in self.get_project_managers():
    ...
    def create_invitation_message(self, inviting_user, pending_user, invite_link):
        ...
        html_builder.set_translations(
            ...
            platform_name=self.deployment_full_name,
            platform_short_name=self.deployment_short_name,
            support_email=SUPPORT_EMAIL,
        )
        html = html_builder.generate_html(
            invite_link=invite_link, support_email=SUPPORT_EMAIL
        )
        msg = EmailMessage(
            subject=subject,
            body=body,
    ...
    def create_new_dashboard_message(
        ...
        subject = f'Your New {deployment_name} Dashboard'
    ...
    def create_alert_notification_message(
        ...
        subject = f'New Alerts for {date_str} from {self.full_platform_name}'
        body = NEW_ALERTS_BODY_EN

        if self.default_locale in {'pt', 'br'}:
            subject = f'Novos Alertas para {date_str} do {self.full_platform_name}'
    ...
    def create_data_upload_alert_message(
        self, new_filenames, deleted_filenames, category, datestamp, uploading_user
    ):
        cc = [user.username for user in self.get_project_managers()]
        to_addr = cc[0]

        subject = (
            f'{self.deployment_abbreviated_name} Data Upload - {category} - {datestamp}'
        )
        body = ''
        if new_filenames:
            filenames_text = '\n\t'.join(new_filenames)
            body += f'New files for "{category}":\n\t{filenames_text}'
        if deleted_filenames:
            if new_filenames:
                body += '\n\n'
            filenames_text = '\n\t'.join(deleted_filenames)
            body += f'Removed files for "{category}":\n\t{filenames_text}'
