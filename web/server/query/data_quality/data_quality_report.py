...
from web.server.query.data_quality.data_quality_score import (
    score,
...
from web.server.query.data_quality.outliers_report import (
    OutliersReport,
    FAILED_OUTLIERS_REPONSE,
)
from web.server.query.data_quality.types import (
    ProvinceOutlierResponses,
    DimensionLevel,
)
from web.server.query.request import parse_groups_for_query, QueryRequest
...
def build_dimension_level_response(
    df: pd.DataFrame,
    all_report_dates: List[pd.Timestamp],
    no_date_filter_df: pd.DataFrame,
    num_reports_key: str,
    lat_lng_fields: List[str],
    dimension_names: List[str],
    end_interval_date: datetime,
    outliers_response_for_field: Dict[str, ProvinceOutlierResponses],
) -> Dict[str, DimensionLevel]:
    lat_field, lng_field = lat_lng_fields

    # NOTE: The dimension level response is only used to power the map so
    ...
    output: Dict[str, DimensionLevel] = {}
    for key in keys:
        ...
        output[key] = {
            **score(all_report_dates, counts, first_report_date, end_interval_date),  # type: ignore
            'geo': {'lat': lat, 'lng': lng},
...
class DataQualityReport(QueryBase):
    ...
    def __init__(
        ...
        geo_to_lat_lng: Optional[Dict[str, tuple]] = None,
        dimension_parents: Optional[Dict[str, List[str]]] = None,
    ) -> None:
        ...
        # NOTE: Store the original request in case we need to also query for
    ...
    def build_df(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        # NOTE: Need to handle this check here because build_df is called for
    ...
    def build_response(  # type: ignore
        self,
        df: pd.DataFrame,
        no_date_filter_df: pd.DataFrame,
        no_geo_filter_df: pd.DataFrame,
        outliers_response: Any = None,
    ):
    ...
    def get_no_date_filter_df(self) -> pd.DataFrame:
        ...
        # TODO: Update this when we work out a way of getting the first report
        # date for each geography without retriveing all report dates.
        # TODO: Work out a way of seperating the existing time and geographical
    ...
    def get_no_geo_filter_df(self) -> pd.DataFrame:
        # TODO: Work out a way of removing dimension filters from the
    ...
    # pylint: disable=arguments-differ
    def get_response(
        self, include_outliers: bool = False
    ) -> Dict[str, Dict[str, DimensionLevel]]:
        outliers_response = None
        if include_outliers:
            outliers_report = OutliersReport(
                self.original_request, self.query_client, self.datasource
            )
            outliers_response = outliers_report.get_response()
