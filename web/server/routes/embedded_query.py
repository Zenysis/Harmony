...
def slim_query_result_spec_in_place(query_result_spec, view_type):
    ...
    NOTE: This optimization is not strictly necessary for the page to work, it
    just is nice to do.
    '''
...
def viz_type_to_view_type(viz_type):
    ...
    NOTE: This is only needed so we can optimize the queryResultSpec to be as
...
def build_dashboard_response(specification, tile_id):
    ...
    # TODO: Handle case where ID is wrong.
    if not item_holder:
        return {}
    ...
    # NOTE: This will definitely break when common settings is changed (which
...
class EmbeddedQueryPageRouter:
    ...
    def build_user_query_value(self, query_uuid):
        try:
            # NOTE: GIANT HACK. There is no clean way to make a call to a
