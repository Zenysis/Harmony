...
FREQUENCY_MAPPING = {'day': 1, 'week': 7}
...
class Resampler:
    def __init__(
        ...
        self.frequency = frequency if frequency in FREQUENCY_MAPPING else 'day'

    def _get_date_str(self, date):
        # cache the datetime object to string mapping so that excess
        # calls to strftime aren't necessary.
        # NOTE: Building our own date hash so that we can avoid storing
        # the full datetime object in the dict. This saves a considerable amount
        # of time and memory.
        # NOTE: String hashes seemed to perform better than tuples as
        # a dict key during profiling.
        date_hash = f'{date.year}-{date.month}-{date.day}'
    ...
    def read_lines(self, lines):
        ...
        for line in lines:
            ...
            for field, value in data.items():
                row_key = f'{row_key_prefix}__{field}'
    ...
    def build_output_rows(
        ...
        # upsample the rows to a (daily or weekly) rate for each key, until the last_date specified.
        one_time_chunk = timedelta(days=FREQUENCY_MAPPING[self.frequency])
        for date_to_row_dict in self._rows_to_resample.values():
            ...
            for i, item in enumerate(rows):
                ...
                end_date = alternate_end_date_function(
                    ...
                    if i < row_count
                    else (self.last_date + one_time_chunk),
                )
