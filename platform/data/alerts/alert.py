...
# TODO: Figure out how to not need to duplicate this from the frontend.
...
def get_day_interval_bounds(latest_date: AlertLatestDate) -> str:
    ...
    # NOTE: Currently implemented so that we only search the day of
...
def get_week_interval_bounds(latest_date: AlertLatestDate) -> str:
    ...
    # NOTE: The end time boundaries is exclusive.
...
def get_month_interval_bounds(latest_date: AlertLatestDate) -> str:
    ...
    if min_date.month == max_date.month:
        ...
        # NOTE: Timedelta can't do months, so do this manually
        end_date = (
...
def get_interval_bounds(latest_date: AlertLatestDate, time_granularity) -> str:
    ...
    NOTE: Druid intervals are inclusive, exclusive, so end time needs to
    account for that.
    '''
    # TODO: Implement other granularities
...
def run_checks(
    ...
    '''
    # TODO: Possibly see if we can implement filtering using
...
def _get_filters(field: Field, alert_def: AlertDefinition) -> Filter:
    ...
    # NOTE: If indicator level filters are enabled for alerts, revisit this as
...
# TODO: Pull out the last date calculation and move it to a util method
