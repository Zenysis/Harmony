...
def get_current_datasource_for_site(
    ...
    try:
        # TODO: It'd be awesome if the version was stored as part of
...
def task_contains_new_data(
    ...
    files: List[Union[str, MinioObject]],
    task_hash_dir: str,
    storage: str = 'local',
) -> bool:
    ...
    # NOTE: This should only happen on a newly setup pipeline system.
    ...
    # difference.
    # NOTE: Intentionally not using a set here since it's possible
    ...
    if storage == 'minio':
        new_file_hash = sorted([f.ETag for f in files])  # type: ignore[union-attr]
    else:
...
def run_task(
    task_definition: dict, index_url: str, dry_run: bool = False, druid_config=None
) -> Optional[str]:
    ...
    druid_params = get_druid_request_params(druid_config)
    r = requests.post(index_url, json=task_definition, **druid_params)
    if not r.ok:
        LOG.error('Failed to start task. Reason: %s; %s', r.reason, r.text)
        return None
    return r.json()['task']
...
def write_task_metadata(
    ...
    TODO: If we need more task information, we should add it as a single
