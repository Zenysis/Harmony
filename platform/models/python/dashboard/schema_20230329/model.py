...
class DimensionValueHierarchy:
    dimension = related.StringField()
    values = related.SequenceField(str, [])


@related.mutable(strict=True)
class FilterCategoryHierarchy:
    category = related.StringField()
    dimensions = related.SequenceField(DimensionValueHierarchy, [])
...
class CommonQueryTileSettings:
    ...
    # NOTE: Active work is still in progress on the models for this
...
@related.mutable(strict=True)
class FilterSettings(CommonQueryTileSettings):
    enabled_filter_hierarchy = related.SequenceField(
        FilterCategoryHierarchy, [], key='enabledFilterHierarchy'
    )


@related.mutable(strict=True)
class GroupingSettings(CommonQueryTileSettings):
    # NOTE: This is a temporary fix to allow the frontend to not send
    # enabled_filter_hierarchy for grouping_settings because currently
    # we have only implemented it in the filter_settings only.
    pass
...
class DashboardCommonSettings:
    ...
    filter_settings = related.ChildField(
        FilterSettings, FilterSettings(), key='filterSettings'
    )
    grouping_settings = related.ChildField(
        GroupingSettings, GroupingSettings(), key='groupingSettings'
    )
...
# TODO: Replace the sequence fields in the AQT query definition
...
# NOTE: DashboardItem is a polymorphic model that can take many types
...
@DashboardItem.register_subtype
@related.mutable(strict=True)
...
# TODO: $GISDashboard - This is going to grow big fast. It would be nice
...
class GISItemDefinition(DashboardItem):
    ...
    # TODO: Update this model to use the QueryResultSpecDefinition and
...
@DashboardItem.register_subtype
@related.mutable(strict=True)
class DividerItemDefinition(DashboardItem):
    '''Model for divider dashboard items.'''

    type = related.StringField('DIVIDER_ITEM')
