...
// TODO: When locale comparison becomes necessary,
// implement it
...
export function normalizeFloatInput(value: ?string): string {
  ...
  // NOTE: keep a negative symbol in the first position, else remove them
  v = v.slice(0, 1) + v.slice(1).replace(/-/g, '');
  // NOTE: keep only one period
  let i = 0;
  v = v.replace(/\./g, m => (!i++ ? m : ''));
  return v;
}
...
/**
 * Slugify a string on a given separator.
 *
 * @param {string} name String to slugify
 * @param {string} slugSeparator Character to use as separator
 * @param {boolean} shortenStr Flag whether to shorten `name` to a max length.
 * (Shortening is best practice but some applications require full string)
 * @returns {string}
 */
export function slugify(
  ...
  const INVALID_CHAR_REGEX = new RegExp('[^a-zA-Z0-9\\s]', 'g');
  ...
  return name
    ...
    .substr(0, strLength)
    .trim()
    .normalize('NFC') // Normalize back to composed form
    .replace(MULTI_SPACE_REGEX, slugSeparator);
}
