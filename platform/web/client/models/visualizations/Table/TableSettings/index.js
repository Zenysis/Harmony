...
type SerializedTableSettings = {
  ...
  rowHeight: number,
  tableFormat: 'table' | 'scorecard',
  // NOTE: this object isn't created automatically in the spec
  userSort: {
    sortColumn: string,
    sortDirection: SortDirection,
  } | void,
...
class TableSettings extends Zen.BaseModel<TableSettings, {}, DefaultValues>
  ...
  serialize(): SerializedTableSettings {
    ...
    return {
      ...
      enableAutoExpand: this._.enableAutoExpand(),
  ...
  updatePivotedDimensionsFromNewGroupBySettings(
    ...
    const newPivotedDimensions = pivotedDimensions.filter(
      id => groupingColumns.has(id) || id === FIELD,
    );
