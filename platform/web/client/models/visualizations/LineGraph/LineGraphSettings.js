...
// TODO: Remove showDataLabels, it has been moved to series settings.
...
const [DEFAULT_GRANULARITY, USE_ET_DATES] = (() => {
  // NOTE: Need to guard this property access since JSON_FROM_BACKEND
  // is not fully populated on all pages (like /login).
  // TODO: Figure out why the login page even loads viz code.
  const {
...
class LineGraphSettings
  ...
  updateFromNewSeriesSettings(
    ...
    this._.bands().forEach(band => {
      ...
      // NOTE: Removal just means not adding it to the new bands list.
      if (
