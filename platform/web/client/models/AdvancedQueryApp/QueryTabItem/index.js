...
function buildTabName(): string {
  TABS_CREATED++;
  return `${I18N.text('Query')} ${TABS_CREATED}`;
}

function extractTabNumber(name: string): number {
  if (!name.startsWith(I18N.textById('Query'))) {
    return -1;
  }

  const tabNumber = Number(name.replace(I18N.textById('Query'), ''));
...
class QueryTabItem
  ...
  static deserializeAsync(
    ...
    return QuerySelections.deserializeAsync(querySelections).then(
      deserializedQuerySelections => {
        // NOTE: Keep the TABS_CREATED variable in sync during
  ...
  // TODO: this is not type safe. We should consider adding an API-level
  ...
  set(key: any, value: any): Zen.Model<QueryTabItem> {
    ...
    // NOTE: Checking `instanceof` vs `key !== 'querySelections'`
