...
// TODO: move everything here to use Zen.Array and Zen.Map, that way we
...
class QueryResultSpec
  ...
  static deserialize(
    ...
    const groupBySettings = GroupBySettings.deserialize(
      serializedObject.groupBySettings,
    );
  ...
  updateSeriesSettingsFromNewFields(
    ...
    return this._.viewTypes().reduce(
      (spec: Zen.Model<QueryResultSpec>, viewType) => {
        ...
        // NOTE: Concatenating previous field IDs here in case they
        ...
        // by the user.
        // NOTE: Skipping this check if there are custom fields since
        // it is difficult to determine the original ordering of fields + custom
        // fields.
        // TODO: Implement better dirty checking so that we
        // don't have to guess.
  ...
  updateVisualizationControlValue(
    ...
    let newSpec = this.updateVisualizationSettings(viewType, vizSettings =>
      vizSettings.updateVisualizationControlValue(controlKey, value),
    );
    if (controlKey === 'pivotedDimensions') {
      const fitWidth = value.length === 0 || value.includes('field');

      newSpec = newSpec.updateVisualizationSettings(viewType, vizSettings =>
        vizSettings.updateVisualizationControlValue('fitWidth', fitWidth),
      );
    }
    return newSpec;
  }
   ...
   *
   * NOTE: However, we only care about changing values if custom fields
