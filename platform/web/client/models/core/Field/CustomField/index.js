...
class CustomField extends Zen.BaseModel<CustomField, RequiredValues>
  ...
  // NOTE: Need access to a version of SeriesSettings that we can
  ...
  static deserialize(
    ...
    const { fieldConfigurations, fieldIds, formula, name } = customFieldObj;
    const seriesObjects = extraConfig.seriesSettings.seriesObjects();
    // TODO: this is not a correct deserialization and will cause issues
    ...
    const fields = fieldIds.map(id => {
      ...
      // NOTE: Never let series objects have an empty label. This will
  ...
  updateInternalFieldLabel(
    ...
    if (fieldToUpdate) {
      ...
      const { labelToTokenMap, lines } = formulaMetadata.tokenizeLines();
  ...
  serialize(): SerializedCustomField {
    const { formula, id, label } = this.modelValues();
    return {
      id,
      fieldConfigurations: formula
        .metadata()
