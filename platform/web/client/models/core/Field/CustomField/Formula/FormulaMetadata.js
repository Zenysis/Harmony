...
type DefaultValues = {
   ...
   * 'timestamp'.
   * NOTE: for now, this array includes *all* dimensions that the user
...
type SerializedFormulaMetadata = {
  // NOTE: on 09/02/2020 this feature was added to Custom Calculations.
...
class FormulaMetadata
  ...
  static deserialize(
    { fieldConfigurations, formula }: SerializedFormulaMetadata,
    ...
    // NOTE: This is duplicated with _sortFields except it uses ID
    ...
    const sortedFields = fieldsToSort.slice().sort((a, b) => {
      ...
      if (aID.length === bID.length) {
        // NOTE: Slightly unintiutive locale compare of `b` vs `a`
    ...
    // NOTE: on 09/02/2020 the `treatNoDataAsZero` feature was added
  ...
  tokenizeLines(): {
    ...
    // NOTE: We need to sort by longest fields (because
    ...
    // NOTE: another hack. This is a mess. We still have to account
    ...
    // identifier.
    // TODO: completely redesign how formulas are represented.
    ...
    const fieldLabelToToken = labels.reduce((map, label) => {
      ...
      // TODO: This is all haappening because we store field labels
    ...
    return { labelToTokenMap: fieldLabelToToken, lines: newLines };
  }
  ...
  getJSFormulaText(): string {
    ...
    const { labelToTokenMap, lines } = this.tokenizeLines();
  ...
  getHTML(mode: 'editor' | 'viewer'): string {
    ...
    const newLines = this._.lines().map((line, lineNumber) => {
      ...
      // NOTE: sort longer fields first, so we don't match the field
      ...
      const lineParts = [];
      let currIndex = 0;
      replacements.forEach(({ endIndex, replacement, startIndex }) => {
  ...
  addField(
    ...
    // TODO multiple lines doesn't seem to work in general
    ...
    // TODO: this was commented out a while ago by Kyle, figure out why
