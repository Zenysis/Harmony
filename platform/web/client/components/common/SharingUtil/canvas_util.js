...
// NOTE: Put this in a more appropriate folder since it is pretty
// generalized.
...
// NOTE: Giant enormous hack! The horizontal bar graph uses a
...
svgAsPngLib.prepareSvg = (el, options, done) => {
  ...
  return svgAsPngLib
    ...
    .then(({ height, src, width }) => {
      ...
      const output = { height, width, src: rewrittenSVGStr };
...
// NOTE: Currently we only care about font related styles.
...
function renderSVG(elt: SVGSVGElement, originElt: HTMLElement) {
  ...
  // NOTE: The save-svg-as-png library does *not* copy over styles
  ...
  // NOTE: Plotly has a really, really annoying "feature" where they
  ...
  // NOTE: This will mutate the element directly inside the SVG. If the
  ...
  return new Promise((resolve, reject) => {
    ...
    // NOTE: Need to preserve the viewBox settings when serializing the
    // SVG. The library doesn't automatically read them from the element.
    const options = {
      modifyCss,
      encoderOptions: 1,
      excludeUnusedCss: true,
      // NOTE: Firefox has a bug where it does not define a default viewBox.
...
// NOTE: The html2canvas library does not detect when fonts have
...
// NOTE: This relies on an undocumented feature of html2canvas's
