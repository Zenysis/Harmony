...
import { downloadFile, uniqueId } from 'util/util';
...
export default class ShareQueryModal extends React.PureComponent<Props, State> {
  state: State = {
    ...
    queryHash: '',
    selectedTabName: this.props.presenting
      ? I18N.textById('Export data')
      : I18N.text('Email'),
  ...
  componentDidMount() {
    ...
    if (show && queryResultSpec && visualizationType !== undefined) {
      if (!presenting && !this.state.loadingShareableLink) {
        this.updateShareURL(
          queryResultSpec,
          querySelections,
          viewType,
  ...
  updateShareURL(
    ...
    // NOTE: Accessing this.setState inside this function is safe even
    ...
    return QuerySessionService.storeQuerySession(
      queryResultSpec,
      querySelections,
      viewType,
      visualizationType,
      ...
      .then(queryHash => {
        this.setState({ queryHash });
        return `${document.location.origin}${localizeUrl(
          '/advanced-query',
        )}#h=${queryHash}`;
      })
      .then(shareableLink => {
  ...
  // NOTE: Due to the mix of controlled and uncontrolled components
  ...
  // TODO: The flow of all of this is not great. It is impossible
  ...
  clearAttachmentsIfNeeded(
    ...
    // NOTE: It is ok to access `this.setState` inside this memoized
    // function.
  ...
  sendEmail(isPreview: boolean) {
    ...
    SendEmailService.sendQueryDataEmail(
      subject,
      recipients,
      sender,
      message,
  ...
  exportFieldMapping(): void {
    ...
    window.location.href = '/api/field_names';
    Toaster.success(
      I18N.text(
  ...
  // TODO: revise this to allow each tab have a custom
  ...
  @autobind
  onExportRawCSVClick() {
    const { downloadRawCSVData } = this.state;

    if (!downloadRawCSVData) {
      return;
    }
    const { queryResultSpec, querySelections } = this.props;
    if (queryResultSpec && querySelections) {
      ...
      // clean it up later if needed
      const promiseId = uniqueId();
      this._queryPromises[promiseId] = TableQueryResultState.runRawQuery(
        querySelections,
        queryResultSpec,
      )
        .then(queryResultState => {
          ...
          const rawCSVData = new Blob([result.content], { type: 'csv' });
          downloadFile({ dataBlob: rawCSVData, fileName: result.filename });

          return result;
        })
        .finally(() => this.setState({ downloadRawCSVData: false }));
    }
  }
  ...
  renderDownloadImageTab(): React.Element<typeof Tab> {
    ...
    return (
      <Tab className="download-image-tab" name={I18N.text('Download image')}>
        <DownloadImageTab
  ...
  renderShareLinkTab(): React.Element<typeof Tab> | null {
    const { loadingShareableLink, presenting } = this.state;

    // do not render the link tab if in presenting mode
  ...
  renderShareEmailTab(): React.Element<typeof Tab> | null {
    ...
    // do not render the email tab if in presenting mode
    if (queryResultSpec && !presenting) {
      return (
