...
export default function PaletteControl({
  ...
  const onPaletteColorChange = React.useCallback(
    (newColor: string, idx: number) => {
      // NOTE: This shouldn't happen, but just to be safe, make sure we
      ...
      <Group.Horizontal spacing="s">
        {palette.map((color, idx) => (
          <ColorBlock
            ...
            color={color}
            enableColorPicker
            onColorChange={({ hex }) => onPaletteColorChange(hex, idx)} // eslint-disable-line react/no-array-index-key
