...
function computeTitleHeight(axis: PlotlyAxisLayout | void): number {
  ...
  // NOTE: Including a little extra padding on top of the title font
...
function computeXAxisHeight(xAxis: PlotlyAxisLayout): number {
  ...
  const { values } = xAxis.tickvals.reduce(
    (acc, val) => {
      // NOTE: Need to remove any spacing on either side of the value
  ...
  // NOTE: Need to include the font size in the calculation since it
...
function getDefaultYAxisRange(
  ...
  // NOTE: Flow cannot correctly refine a numeric type using
  // Number.isFinite so we are suppressing the error.
  // $FlowIssue[incompatible-return]
  return { range, fixed: true };
}
...
function computeYAxisWidth(
  ...
  // NOTE: Plotly will draw the y-axis title rotated 90 degrees, so we
