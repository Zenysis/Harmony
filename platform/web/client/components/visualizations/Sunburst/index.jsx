...
function getAncestors(node: D3SunburstNode): $ReadOnlyArray<D3SunburstNode> {
  ...
  while (current.parent) {
    // NOTE: While loop would catch this but Flow doesn't notice.
...
class Sunburst extends React.PureComponent<Props, State> {
  ...
  render() {
    return (
      <Visualization footer={this.renderFooter()} loading={this.props.loading}>
        <div ref={this.resizeRegistration.setRef} className="sunburst" />
      </Visualization>
    );
  }
}
