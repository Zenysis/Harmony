...
export default function ExpandedTimeline({
  ...
  // TODO: There is a bug where the window will register that a user
    ...
    return (
      <React.Fragment>
        <Group.Horizontal
          ...
          spacing="none"
        >
          {/** NOTE: Render a hidden icon so that the play button
           * controls can properly be centered */}
          <Icon style={{ visibility: 'hidden' }} type="svg-settings-outline" />
          ...
          <div ref={settingsButtonAnchorRef} style={{ marginBottom: '-4px' }}>
    ...
    return (
      <Group.Horizontal flex justifyContent="space-between" marginRight="xxxs">
        ...
        >
          ...
          >
            {dates.map((date, idx) => (
              <Dropdown.Option key={date} value={idx}>
                {date}
              </Dropdown.Option>
            ))}
          </Dropdown>
  ...
  // NOTE: The width passed into InputSlider is a width that represents
