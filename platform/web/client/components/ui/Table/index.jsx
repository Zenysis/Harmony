...
> {
    ...
    if (sortedData === undefined) {
      ...
      const newData = [...data].sort((a, b) => {
        ...
        if (sortResult === 0 && otherSortFns) {
          // NOTE: for loop is intentional for performance
  ...
  getSearchKeywordsPerRow(): Map<RowData, $ReadOnlyArray<?string> | void> {
    const { data, getSearchKeywords, headers } = this.props;
  ...
  maybeRenderTableHeader(): React.Node {
    const { headers, isEditable, showHeaders } = this.props;
    const { sortInfo } = this.state;
  ...
  renderRows(): React.Node {
    ...
    const { currentEditRow, currentPage } = this.state;
    ...
    if (searchText !== '' && rowData.length === 0) {
      ...
      return (
        <tr className="zen-table__row">
          <TableCell className={noResultsClassName} colSpan={headers.length}>
            {noSearchResultsText}
          </TableCell>
        </tr>
      );
    ...
    return finalData.map<React.Element<typeof TableRowWrapper> | null>(
      (row, idx) => {
        ...
        if (rowElement !== undefined && rowElement !== null) {
          ...
          const { className, disableClick, id, isSelected } = rowElementProps;
