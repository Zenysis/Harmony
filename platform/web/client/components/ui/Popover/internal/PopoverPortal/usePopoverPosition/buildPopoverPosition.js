...
function _buildPopoverPosition(left: number, top: number): PopoverPosition {
  // NOTE: Sometimes popover positions that are not integers will cause
...
function _getAnchorPosition(
  ...
  const { left, top } = getPositionRelativeToDocument(anchorElt);
  return {
 ...
 */
// TODO: this calculation involves repeated calls to
...
export default function buildPopoverPosition(
  ...
  if (newOrigins.popoverOrigin !== popoverOrigin) {
    ...
    if (_didOriginsFlipVertically(popoverOrigin, newOrigins.popoverOrigin)) {
      ...
      if (
        ...
        // TODO: there is a lot of duplicate logic here. Figure out how
  ...
  // NOTE: our final calculated position might *still* not fit in the
