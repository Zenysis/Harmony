...
function buildVerticalPosition(
  ...
  // NOTE: If we are stacking bars, continuously update the start
  ...
  metrics.forEach((_, idx) => {
    ...
    // NOTE: The coordinate system can be a little confusing. The
    ...
    // NOTE: If the bar height was clambed to the minimum height, and
...
export default class BarGroup extends React.PureComponent<Props> {
  ...
  buildMetricOrderByAxis(
    ...
    if (dataPoint !== undefined) {
      // NOTE: Taking the absolute value since we care about bar
      // length.
  ...
  buildOverlappingBarsPositionMap(
    ...
    // width.
    // NOTE: This is a best guess at a clean way for applying overlap
  ...
  render(): $ReadOnlyArray<React.Element<typeof Bar>> {
    // NOTE: add bars to DOM in reverse order to ensure that text
    // annotations do not get covered up by a subsequent bar.
    // NOTE: If the bars are overlaid or overlapping, we want to add to
