...
// TODO: consider using full MetricAxis component
...
// TODO: fix once scale types are defined
...
export const DEFAULT_THEME: EpiCurveTheme = {
  // NOTE: epicurve only displays one series so all bars have the same
...
export default class EpiCurveCore extends React.PureComponent<Props, State> {
  ...
  // NOTE: right padding added because the responsive container doesn't
  ...
  getPadding(): Padding {
    ...
    // NOTE: approximate values to display text with width <= 40px
  ...
  getBarStyle(timestamp: string): BarStyle {
    ...
    return {
      ...
      // NOTE: make borders low opacity so they blend into bar
      strokeOpacity: 0.1,
    };
  }
  ...
  renderAxisBottom(): React.Element<typeof AxisBottom> {
    // TODO: add collision detection for labels
    const { axisTitles, theme, timeFormatter } = this.props;
    const xScale = this.getXScale();
  ...
  renderInnerChart(): React.Element<'g'> {
    ...
    // NOTE: Render the bars in reverse order to ensure that text
