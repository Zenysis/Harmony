...
export type AQTAction =
  ...
  | { querySelections: QuerySelections, type: 'QUERY_SELECTIONS_CHANGE' }

  /** Sets a new query result spec */
  | { newSpec: QueryResultSpec, type: 'QUERY_RESULT_SPEC_CHANGE' }
  ...
  | { show: boolean, type: 'VIZ_PICKER_EXPLORE_VIEW_TOGGLE' }
  ...
  | { newTab: QueryTabItem, type: 'NEW_TAB_ADD' }
  ...
  | { newTabIdx: number, type: 'CURRENT_TAB_CHANGE' }
...
export function appReducer(state: AQTState, action: AQTAction): AQTState {
  ...
  switch (action.type) {
    case 'QUERY_SELECTIONS_CHANGE': {
      ...
      // NOTE: If there are no fields selected, we need to clear
      ...
      if (
        ...
        // NOTE: If a user has "locked" on a visualization at this point,
    ...
    case 'VISUALIZATION_TYPE_CHANGE': {
      ...
      return {
        ...
        // NOTE: We want to clear out the "locked" viz selection, since
    ...
    case 'VISUALIZATION_AND_SPEC_CHANGE': {
      const { newSpec, visualizationType } = action;
