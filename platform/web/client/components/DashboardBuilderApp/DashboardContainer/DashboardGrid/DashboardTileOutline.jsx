...
type Props = {
  ...
  resizing: boolean,
  selectableTileIds: $ReadOnlyArray<string>,
...
// NOTE: This resize handle isn't actually hooked up. We just draw it
...
function DashboardTileOutline({
  ...
  // NOTE: We need to draw the highlighted tiles last so that the
  ...
  function renderTileOutline(tile: ReactGridPosition) {
    // NOTE: since we want to show a tile outline for query tiles,
    // we always pass the query tile ids when presenting mode is active.
    // If a tile's id is not in selectableTileIds, we do not highlight it.
    // (for ex. text tiles, space tiles, etc)
    if (activateTileDownloads && !selectableTileIds.includes(tile.i))
      return null;
    ...
    const resizeHandles = tile.resizeHandles
      ? new Set(tile.resizeHandles)
      : undefined;
    ...
    // NOTE: Setting a dash offset ensures that no matter where a tile
    ...
    // size of the gap.
    return (
      <g key={tile.i} className={className}>
        <rect
        ...
        {tileIsSelected && (
          <>
            {(!resizeHandles || resizeHandles.has('w')) &&
              renderResizeHandle(x1, y1 + height / 2 - 3)}
            {(!resizeHandles || resizeHandles.has('e')) &&
              renderResizeHandle(x2, y1 + height / 2 - 3)}
            {(!resizeHandles || resizeHandles.has('sw')) &&
              renderResizeHandle(x1, y2)}
            {(!resizeHandles || resizeHandles.has('s')) &&
              renderResizeHandle(x1 + width / 2 - 3, y2)}
            {(!resizeHandles || resizeHandles.has('se')) &&
