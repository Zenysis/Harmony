...
/**
 * NOTE: we can expand the number of filter types that can extract
 * from the filter settings by passing in more types here,
 * for example `DIMENSION_VALUE`
 */
const FILTER_ITEM_TYPES = ['CUSTOMIZABLE_TIME_INTERVAL'];

type Props = {
  /**
   * Whether or not to activate dashboard tile download
   * menus
   */
...
// NOTE: Setting this to 0,0 since we apply our own padding between
...
function getAppliedFiltersAndGroupingsForTile(
  ...
  const filterableItems = ['QUERY_ITEM', 'GIS_ITEM'];
  if (
    filterableItems.includes(item.tag) &&
...
function DashboardGrid({
  ...
  const containerStyle = React.useMemo(
    () => ({
      ...
      // NOTE: In legacy mode, we set a max-width instead of using the
  ...
  const [heightsOverrides, setHeightsOverrides] = React.useState(new Map());
  const onHeightUpdate = React.useCallback(
    (id: string, rowCount: number) => {
      setHeightsOverrides(
        new Map(heightsOverrides.entries()).set(id, rowCount),
      );
    },
    [heightsOverrides, setHeightsOverrides],
  );
  ...
  // When in presenting mode, we need to intercept hoveredTile and selectedTile and
  // only allow specific tiles to be hovered over/selected
  const selectableTileIds = React.useMemo(() => {
    if (activateTileDownloads) {
      return items
        .filter(item => item.item().tag === 'QUERY_ITEM')
        .map(item => item.id());
    }
    return [];
  }, [activateTileDownloads, items]);

  // If we are not in collapse mode, then only the selected tile
  // should have buttons shown. In legacy mode, the hover tile should have them
  // shown.
  // NOTE: we have a special case where we need to show tile buttons in
  // preseting mode and active tile downloads is enabled.
  // This is because we need to allow users to download analysis data from the tile.
  let tileWithVisibleButtons;
  if ((editing || activateTileDownloads) && !repositioningTiles) {
    tileWithVisibleButtons = legacy ? hoverTileId : selectedTileId;
  }

  const filterSettings = commonSettings.filterSettings();
  const extractedFilterItems = filterSettings.items.filter(settingsItem =>
    FILTER_ITEM_TYPES.includes(settingsItem.tag),
  );
  ...
  // NOTE: ReactGridLayout recommends memoizing the children array so
      ...
      items.map(item => {
        ...
        const tileContainer = (
          <TileContainer
            ...
            overriddenRowCount={heightsOverrides.get(item.id())}
            padding={tilePadding}
        ...
        if (IS_IFRAME_REQUEST) {
          // NOTE: this is was so because since am setting the height to 100vh in the dashboard,
          // the lazyloading was not picking the components.
          // As explained in the utility function here https://www.npmjs.com/package/react-lazyload
          forceCheck();
        }
  ...
  // NOTE: Must wrap the handle in an element because the resize icon
  ...
  // NOTE: It would be nice to define the position of each tile
  ...
  // NOTE: Legacy dashboards do not use vertical compacting. Modern
  ...
  return (
    <React.Fragment>
      <div className={gridClassname} id="grid-layout" style={containerStyle}>
        ...
        {(editing || activateTileDownloads) && !legacy && (
