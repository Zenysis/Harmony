...
type Props = {
  ...
  >,
  horizontal: boolean,
  onSettingsChange: (FilterSettings<QueryFilterItem>) => void,
  presenting: boolean,
  settings: FilterSettings<QueryFilterItem>,
};
...
function CommonFilterSettings({
  ...
  const { enabledFilterHierarchy, excludedTiles, items } = settings;
  const enabledCategories = enabledFilterHierarchy.map(
    hierarchy => hierarchy.category,
  );
  const [dimensions, , dimensionValueMap] = useSupportedItems(
    [],
  ...
  // is selected.
  // NOTE: The date range category id is included in the
  ...
  // NOTE: The existing DashboardFilterSelector expects values to come
  ...
  const onFilterItemsChange = React.useCallback(
    newSelectedItems => {
      ...
      // NOTE: This is different from the existing GridDashboardApp
