 ...
 *
 * NOTE: This hook violates some of the rules of hooks by introducing
...
export default function useOneTimeRequest<T>(
  ...
  React.useEffect(() => {
    ...
    if (currentRequestState.loadState.complete) {
      // NOTE: Only call `setState` if the value has changed from the
    ...
    // NOTE: Disabling this lint here because there is no reason to
