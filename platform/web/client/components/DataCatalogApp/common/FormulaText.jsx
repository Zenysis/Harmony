...
export default function FormulaText({
  ...
  function renderToken({ type, value }: Token, idx: number) {
    ...
    if (type === 'characters') {
      return (
        <div key={key} className="formula-text__token-characters">
          {value}
        </div>
      );
    }
    ...
    if (!enableFieldClick) {
      return (
        <div key={key} className="formula-text__token-field">
          {name}
        </div>
      );
    }

    // TODO: Should we allow clicking out to fields that do not have a
    // name mapped?
    // NOTE: The field ID stored inside the calculation is the
