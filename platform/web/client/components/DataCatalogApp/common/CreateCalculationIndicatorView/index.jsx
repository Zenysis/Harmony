...
export default function CreateCalculationIndicatorView({
  ...
  // NOTE: Including the `ParentCategoryChange_fieldCategoryMapping`
  ...
  // server.
  // NOTE: field_dimension_mappings is returned in the response even
  ...
  const calculationButtonsCaptions = {
    FORMULA: I18N.text('formula'),
  };
  const calculationButtons = ['FORMULA'].map(type => {
    return (
      <Button key={type} onClick={() => onCreateCalculationClick()} outline>
        {calculationButtonsCaptions[type]}
      </Button>
    );
  });
  ...
  return (
    <React.Fragment>
      <BaseModal
        ...
        height={780}
        maxWidth={790}
        onPrimaryAction={onSaveCalculation}
