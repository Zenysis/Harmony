...
// NOTE: I really wanted this to be a generator function! Babel
...
function findMatches(
  ...
  while (match !== null) {
    // NOTE: Try to prevent infinite loops in case a bad regex is
    // passed in.
    if (loopCount >= 10000) {
...
function splitCharacterPieces(characters: string): $ReadOnlyArray<Token> {
  ...
  matches.forEach(({ matchValue, stringSinceLastMatch }) => {
    ...
    if (matchValue !== undefined) {
      ...
      output.push(
        ...matchValue.split('').map(value => ({ value, type: 'characters' })),
      );
    }
  });
  return output;
