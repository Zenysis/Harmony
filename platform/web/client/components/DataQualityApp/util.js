...
export const URL_PARAMS = {
  // NOTE: This arg still refers to a geo filter so as not to break
...
function scoreInfo(
  ...
  return { color, displayText, lowerThreshold, upperThreshold };
}
...
export function getScoreInfo(score: number, maxScore: number): ScoreInfo {
  ...
  if (normalizedScore > SCORE_THRESHOLDS.GOOD) {
    return scoreInfo(
      SCORE_THRESHOLDS.GOOD,
      SCORE_THRESHOLDS.GREAT,
      // NOTE: Whilst semantically using "hover" colors feels a bit
...
export function getTimeIntervalFromURL(
  ...
  return TimeInterval.create({ end, start });
}
...
export function getDQLURL(
  ...
  // NOTE: Currently DQL only allows one time and one dimension value
