...
export type ResizeRegistration<T: Element = Element> = {
  ...
  // NOTE: This method is only needed for advanced usages of the
...
class ElementResizeService {
  ...
  register<T: Element>(
    ...
    this.registrations.set(id, { onResize, currentElement: null });
  ...
  onResize(entries: $ReadOnlyArray<ResizeObserverEntry> = []) {
    ...
    // NOTE: Resize callbacks are processed inside an animation frame
    // to avoid a "loop limit exceeded" error with the underlying
    // ResizeObserver.
    // TODO: Provide `immediate` mode if needed.
