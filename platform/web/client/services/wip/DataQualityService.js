...
const OUTLIERS_BOX_PLOT_ENDPOINT = 'query/outliers_box_plot';
const OUTLIERS_TABLE_ENDPOINT = 'query/outliers_table';
const OUTLIERS_LINE_GRAPH_ENDPOINT = 'query/outliers_line_graph';
...
class DataQualityService {
  ...
  ): Promise<BoxPlotQueryResultData> {
    const queryFilters = this._getCombinedFilters(filters);

    return GranularityService.forceGet('day').then(granularity => {
      const groupings = [
        GroupingGranularity.createFromGranularity(granularity),
        this._createGroupingDimension(geographyGroupBy),
      ];
      ...
      return Query.create(
        OUTLIERS_BOX_PLOT_ENDPOINT,
        {
          outlierType,
          queryRequest: selections.serializeForQuery(),
        },
        API_VERSION.V2,
      )
        .run()
        .then(BoxPlotQueryResultData.deserialize);
    });
  }

  @autobind
  getOutliersLineGraph(
    field: Field,
    geographyGroupBy: Dimension,
    filters: Filters,
    outlierType: OutlierType,
  ...
  getOutliersTable(
    ...
    return GranularityService.forceGet('day').then(granularity => {
      const groupings = [
        GroupingGranularity.createFromGranularity(granularity),
        ...geographyGroupBys.map(geographyGroupBy =>
          this._createGroupingDimension(geographyGroupBy),
        ),
      ];
      ...
      return Query.create(
        ...
        .run()
        .then(TableQueryResultData.deserialize);
    });
  }
}
