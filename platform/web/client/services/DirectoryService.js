...
class DirectoryService {
        ...
        .then(() =>
          resolve(
            window.toastr.success(
              I18N.text('Group users were successfully updated'),
            ),
          ),
        )
        .catch(error => reject(error));
  ...
  createGroup(group: SecurityGroup): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this._httpService
        ...
        .then(() =>
          resolve(
            window.toastr.success(I18N.text('Group was successfully created.')),
          ),
        )
        .catch(error => reject(error));
    });
  ...
  updateGroup(group: SecurityGroup): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this._httpService
        ...
        .then(() =>
          resolve(
            window.toastr.success(I18N.text('Group was successfully updated.')),
          ),
        )
        .catch(error => reject(error));
    });
  ...
  deleteGroup(group: SecurityGroup): Promise<void> {
    return new Promise((resolve, reject) => {
      this._httpService
        ...
        .then(() =>
          resolve(
            window.toastr.success(I18N.text('Group was successfully deleted.')),
          ),
        )
        .catch(error => {
          reject(error);
   ...
   * @param {User} user
   *
   * @returns {Promise<APIToken>} A promise that when completed successfully,
   *                              will contain the newly created token object.
   */
  @autobind
  generateUserAPIToken(user: User): Promise<APIToken> {
    return this._httpService
      .post(API_VERSION.NONE, `${user.uri()}/generate_api_token`)
      .then(token => APIToken.deserialize(token));
  }
}
