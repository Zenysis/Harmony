...
import type { DashboardTilePermissions } from 'models/core/Dashboard/types';
...
// NOTE - Pagination should be supported via a Pagination Service.
...
class DashboardService {
  ...
  getDashboards(
    ...
    // NOTE We need to convert the `+` character found in some emails
    ...
    return new Promise((resolve, reject) => {
      this._httpService
        ...
        .then((updatedDashboard: Dashboard) => {
          ...
          // NOTE: Previously the official status change was handled by
          ...
          // NOTE: As with everything dashboard related, this is
          ...
          // NOTE: In order to update multiple properties about the dashboard,
          // we collect all the calls that need to be made as promises and
          // then resolve them all at once.
          const dashboardUpdatePromises = [];
          if (updatedDashboard.isOfficial() !== dashboard.isOfficial()) {
            const isOfficialPromise = this.markDashboardAsOfficial(
              dashboard,
              dashboard.isOfficial(),
            ).then(() => dashboard);
            dashboardUpdatePromises.push(isOfficialPromise);
          }

          // Check updates for dashboard tile download permissions
          const [currentRegisteredUsersPerm, prevRegisteredUsersPerm] = [
            updatedDashboard.registeredUsersCanDownloadData(),
            dashboard.registeredUsersCanDownloadData(),
          ];
          const [currentUnregisteredUsersPerm, prevUnregisteredUsersPerm] = [
            updatedDashboard.unregisteredUsersCanDownloadData(),
            dashboard.unregisteredUsersCanDownloadData(),
          ];
          if (
            currentRegisteredUsersPerm !== prevRegisteredUsersPerm ||
            currentUnregisteredUsersPerm !== prevUnregisteredUsersPerm
          ) {
            const setTileDataPermissionsPromise = this.setDashboardTileDataPermissions(
              dashboard,
              {
                registered_users_can_download_data: prevRegisteredUsersPerm,
                unregistered_users_can_download_data: prevUnregisteredUsersPerm,
              },
            ).then(() => dashboard);
            dashboardUpdatePromises.push(setTileDataPermissionsPromise);
          }

          // resolve all the promises
          Promise.all(dashboardUpdatePromises).then(() => dashboard);
  ...
  /**
   * Enables/disables permission for registered users to download analysis data
   * from the dashboard.
   *
   * @param {Dashboard} dashboard The dashboard to update.
   * @param {boolean} registeredUsersCanDownloadData Indicates whether or not registered users can
   *                             download analysis data from dashboard
   *
   * @returns {Promise<void>} A promise that when complete will indicate that
   *                          the "official" status of the dashboard has been
   *                          updated.
   */
  @autobind
  setDashboardTileDataPermissions(
    dashboard: Dashboard,
    currentTilePermissions: DashboardTilePermissions,
  ): Promise<void> {
    this._invalidateCaches();
    const requestUri = `${dashboard.uri()}/set_tile_data_permissions`;

    return new Promise((resolve, reject) => {
      this._httpService
        .post(API_VERSION.NONE, requestUri, currentTilePermissions)
        .then(resolve)
        .catch(error => {
          reject(error);
        });
