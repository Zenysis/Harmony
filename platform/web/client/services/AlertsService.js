...
class AlertsService {
  ...
  getAlertDefinition(uriWithPrefix: string): Promise<AlertDefinition> {
    // NOTE: The stored URI already has the api version prefix attached.
  ...
  getAllAlertNotifications(): Promise<Array<AlertNotification>> {
    return new Promise((resolve, reject) => {
      // $CycloneIdaiHack
      // We cannot hit the standard `GET` endpoint
      // since it does not filter instances.
      this._httpService
        .get(
          API_VERSION.V2,
   ...
   * TODO: We are trying to move toward a world where we store
  ...
  getAlertsForDimensionValue(
    ...
    // TODO: Switch the AlertDefinition model
