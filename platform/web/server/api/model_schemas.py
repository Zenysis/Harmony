...
from models.alchemy.dashboard import (
    ScheduleCadenceEnum,
...
def role_list_as_map(roles):
    ...
    # TODO: Fix this, and decide whether is makes sense to also send
    # group IDs
    return output
    for role in roles:
...
# NOTE: This model is not directly exposed because it has many relations and we don't
...
# NOTE: The structure of `GroupRoles` is identical to `UserRoles` although the models
...
RECIPIENTS_PROPERTY = fields.Array(fields.String(), min_items=0)
RECIPIENTS_USER_GROUPS_PROPERTY = fields.Array(
...
DASHBOARD_REPORT_SCHEDULE_PROPERTIES = {
    ...
    'recipients': RECIPIENTS_PROPERTY,
    ...
    'recipientUserGroups': RECIPIENTS_USER_GROUPS_PROPERTY,
}
...
QUERY_LINK_PATTERN = r'#h=([0-9a-f]{32})$'
DASHBOARD_REPORT_GENERATOR_PROPERTIES = {
    'id': fields.Integer(nullable=True),
    'name': fields.String(),
    'queryLink': fields.String(attribute='query_link_id', pattern=QUERY_LINK_PATTERN),
    'recipients': RECIPIENTS_PROPERTY,
    'recipientUserGroups': RECIPIENTS_USER_GROUPS_PROPERTY,
}
DASHBOARD_REPORT_GENERATOR = fields.Object(
    properties=DASHBOARD_REPORT_GENERATOR_PROPERTIES,
)
DASHBOARD_REPORT_GENERATORS = fields.Array(
    alchemy_fields.InlineModel(
        DASHBOARD_REPORT_GENERATOR_PROPERTIES, model=DashboardReportGenerator
    )
)
