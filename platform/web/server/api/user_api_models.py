...
from web.server.api.api_token_models import APITokenResource
...
from web.server.potion.managers import UserResourceManager
...
class UserResource(PrincipalResource):
    ...
    class Meta:
        ...
        manager = principals(UserResourceManager)
    ...
    class Schema:
        ...
        apiTokens = fields.Array(
            fields.Inline(APITokenResource), attribute='api_tokens'
        )
    ...
    @ItemRoute.POST(
        '/generate_api_token',
        rel='generateApiToken',
        title='Generate an API token for a user',
        description='Generates an API token to be used with given user identity',
        response_schema=fields.Inline(APITokenResource),
    )
    def create_api_token_for_user(self, db_user):
        # NOTE: I'm not sure if we need a new permission here to obtain tokens
        # the idea behind reusing `change_password` is that both this and that allow
    ...
    def reset_password(self, user):
        with AuthorizedOperation('reset_password', 'user', user.id):
            ...
            # TODO - This is for legacy users who signed up with usernames that do
...
@after_user_group_change.connect
@after_user_role_change.connect
def invalidate_user_identity_cache(sender, role=None, group=None):
