...
class Error(Response):
    def __init__(self, error_data):
        super().__init__(data=error_data, success=False)


class Success(Response):
    def __init__(self, data=None):
        super().__init__(data=data, success=True)
...
def get_user_string(user, include_ip=True):
    ...
    if user.is_authenticated:
        ip_string = f', IP:{remote_ip_address}' if include_ip else ''

        return f'{user.last_name}, {user.first_name} (Id:{user.username}{ip_string})'

    return f'Anonymous (IP:{remote_ip_address})'


def get_dashboard_title(specification):
    # NOTE - The whole point of implementing versioning is to ensure
...
def get_resource_string(resource_name, resource_type):
    if resource_name:
        return f'resource \'{resource_name}\' of type: \'{resource_type}\''
    return f'all resources of type \'{resource_type}\''
...
def get_enum_values(enum_class):
    return {enum_value.name for enum_value in enum_class}
...
def assert_in(value, all_values, argument_name=None):
    if value not in all_values:
        argument_string = f'for argument \'{argument_name}\'' if argument_name else ''

        raise ValueError(
            f'Value \'{value}\' {argument_string} is not in \'{all_values}\''
        )
...
def element_type_validator(
    ...
    argument_name = (
        f'{argument_name}[{index}]' if argument_name else f'Element \'{index}\''
    )
...
def key_value_element_validator(
    ...
    argument_name = f'{argument_name}[{key}]' if argument_name else f'Key \'{key}\''
...
def try_parse_enum(value, enum_type, argument_name=None, value_converter_function=None):
    ...
    try:
        ...
        raise ValueError(f'Unexpected value for \'{enum_type}\': \'{value}\'')
...
def assert_users_exist(value):
    ...
    if undefined_users:
        raise ValueError(f'Users with ids {undefined_users} do not exist')
...
class CachedRoute(Route):
    """
    A subclass of potion's `Route` that uses `Flask-Caching` to cache the serialized results.
    """

    def __init__(self, *args, cache_kwargs=None, **kwargs):
        super().__init__(*args, **kwargs)
        self._cache_kwargs = cache_kwargs

    def view_factory(self, *args, **kwargs):
        return current_app.cache.memoize(**self._cache_kwargs or {})(
            super().view_factory(*args, **kwargs)
        )
