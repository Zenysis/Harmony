...
def join_str_columns(
    ...
    # NOTE: The na_rep value of an empty string might produce a weird
...
def build_key_column(
    ...
    # TODO: Remove this once it becomes impossible to set incomplete label
    ...
    # NOTE: Dropping duplicates since the query might have had a
    ...
    if len(grouping_dimensions) > 0:
        label_df.set_index(grouping_dimensions, inplace=True)

    return label_df


# NOTE: This is a really dirty workaround for NaN, Infinity,
...
def clean_df_for_json_export(df: pd.DataFrame) -> pd.DataFrame:
...
def collapse_intervals(intervals: List[Tuple[Any, Any]]) -> List[Tuple[Any, Any]]:
...
def find_interval_intersection(
    lhs: List[Tuple[Any, Any]], rhs: List[Tuple[Any, Any]]
) -> List[Tuple[Any, Any]]:
    ...
    for interval_lhs in sorted_lhs:
        for interval_rhs in sorted_rhs:
            ...
            assert intersection[0] < intersection[1], f'Huh: {intersection}'
...
class FilterTree:
    ...
    def _get_tree_intervals(self) -> List[Tuple[Any, Any]]:
    ...
    def to_druid_intervals(self) -> List[str]:
        if self.all_time:
            return []
