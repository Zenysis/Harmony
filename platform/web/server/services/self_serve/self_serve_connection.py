...
class SelfServeConnection:
    ...
    def __init__(
        ...
        # NOTE: Allow the deployment_code to be passed in in case we are using
    ...
    def upload_data_file(self, filepath: str, is_temporary: bool) -> str:
        '''This function uploads data to s3 and returns the s3 filepath.

        Args
        ----
            filepath (str): Path on server to the file to be uploaded to s3
            is_temporary (bool): Whether the file should be set to expire in a day.
        returns
        -------
        ...
        if is_temporary:
            self.connection_manager.upload_file(
                filepath, s3_key, expiration=FileExpiration.ONE_DAY
            )
        else:
    ...
    def update_to_never_expire(self, file_path: str) -> None:
        key = self.get_file_key(file_path)
        self.connection_manager.set_file_expiration(key)

    def delete_files(self, file_names: List[str]) -> None:
        for file_name in file_names:
            file_path = self.get_file_key(file_name)
            self.connection_manager.delete_file(file_path)
