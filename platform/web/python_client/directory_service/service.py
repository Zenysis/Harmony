...
class DirectoryService(ApiService):
    ...
    def get_user(self, username):
        filter_string = f'?where={{"username":"{username}"}}'
        ...
        if len(users) > 0:
            ...
            raise ValueError(f'No user with username \'{username}\' exists.')
    ...
    def _update_user_roles(self, user):
        ...
        destination_uri = self.get_destination_uri(f'{user.uri}/roles')

        return self.patch(destination_uri, json=user.roles.serialize())

    def change_user_password(self, user, new_password):
        data = {'newPassword': new_password}
        destination_uri = self.get_destination_uri(f'{user.uri}/password')
        return self.post(destination_uri, json=data)

    def reset_user_password(self, user):
        destination_uri = self.get_destination_uri(f'{user.uri}/reset_password')
    ...
    def get_group(self, group_name):
        filter_string = f'?where={{"name":"{group_name}"}}'
        ...
        if len(groups) > 0:
            ...
            raise ValueError(f'No group with username \'{group_name}\' exists.')
    ...
    def _update_group_roles(self, group):
        ...
        destination_uri = self.get_destination_uri(f'{group.uri}/roles')
    ...
    def _update_group_users(self, group):
        ...
        destination_uri = self.get_destination_uri(f'{group.uri}/users')
