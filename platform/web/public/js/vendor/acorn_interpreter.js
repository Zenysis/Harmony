// NOTE: We prefer non-minified vendor files in dev to ease debugging.
// Since the JS-Interpreter project does not provide a non-minified version of
// the combo acorn_interpreter.js file, we manually create it here by placing
// acorn.js and interpreter.js here.
// NOTE: Acorn must come first, since it is a dependency of Interpreter
// NOTE: To stay in sync with the minified file, make sure you use the
// acorn.js and interpreter.js files from the same commit as acorn_interpreter.js
...
Interpreter.prototype.initFunction = function(scope) {
  ...
  wrapper = function(var_args) {
    if (thisInterpreter.calledWithNew()) {
      ...
      // Called as Function().
      var newFunc = thisInterpreter.createObjectProto(
        thisInterpreter.FUNCTION_PROTO,
      );
    }
    if (arguments.length) {
...
Interpreter.prototype.initArray = function(scope) {
  ...
  wrapper = function(var_args) {
    if (thisInterpreter.calledWithNew()) {
      ...
      // Called as Array().
      var newArray = thisInterpreter.createObjectProto(
        thisInterpreter.ARRAY_PROTO,
      );
    }
...
Interpreter.prototype.initRegExp = function(scope) {
  ...
  wrapper = function(str) {
    ...
    this.data.lastIndex = Number(
      thisInterpreter.getProperty(this, 'lastIndex'),
    );
    var match = this.data.exec(str);
    thisInterpreter.setProperty(this, 'lastIndex', this.data.lastIndex);

    if (match) {
      var result = thisInterpreter.createObjectProto(
        thisInterpreter.ARRAY_PROTO,
      );
  ...
  if (this.class === 'Array') {
    ...
    try {
      ...
      for (var i = 0; i < this.properties.length; i++) {
        ...
        strs[i] =
          value && value.isObject && cycles.indexOf(value) !== -1
            ? '...'
            : value;
      }
    } finally {
