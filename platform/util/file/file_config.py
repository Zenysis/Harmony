...
class FileConfig:
    ...
    def __init__(self, input_file, output_file, name=None):
        assert (
            input_file and output_file and input_file != output_file
        ), 'Invalid file config specified. ' 'Input file: %s\tOutput file: %s' % (
            input_file,
            output_file,
        )
...
def validate_file_configs(file_configs):
    ...
    for config in file_configs:
        ...
        assert os.path.isfile(input_file), f'Input file does not exist: {input_file}'
        assert (
            input_file not in input_files
        ), f'Input file is scheduled to be processed twice: {input_file}'
        assert (
            output_file not in output_files
        ), f'Output file is scheduled to be written to twice: {output_file}'
